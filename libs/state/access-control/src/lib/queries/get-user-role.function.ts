 ...
 * @returns 
 */
export function _CheckPermission(getPermission: (permission: KujaliPermissions) => KujaliPermissions, permissions$$: PermissionsStore, user$$: any) 
  : Observable<boolean>
{
...
export function _getUserRole(user$$: UserService<KuUser>)
{
  return user$$.getUser()
          .pipe(map((u) => u.roles[u.profile.activeOrg]));
}

/** 
 * Core permission determination algorithm of the Kujali. 
 * 
